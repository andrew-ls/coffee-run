...
export function DrinkPills({ order }: DrinkPillsProps) {
  ...
  return (
    <>
      ...
      {iced && <Pill color={ASPECT_COLORS.iced} label={t('orderCard.drinkSummary.iced')} />}
      {variant && variant !== 'Other' && (
        <Pill color={ASPECT_COLORS.variant} label={t(`drinks.variants.${variant}`, variant)} />
      )}
      {customVariant && <Pill color={ASPECT_COLORS.variant} label={customVariant} />}
      {customDrinkName && <Pill color={ASPECT_COLORS.variant} label={customDrinkName} />}
